#define Muon_count_100_noscint_cxx
#include "Muon_count_100_noscint.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void Muon_count_100_noscint::Loop()
{
    if (fChain == 0) return;
   ...
   std::string text = "No scint | avg # of hits in each layer 100 events (5GeV mu- gun) | total: ";
   text += std::to_string(avg_count_total);
   text += "+/-";
   text += std::to_string(y_err_total);
   cout << "text: " << text << "\n";
   
   std::string mc_text = "No scint | avg # of -22 PDG in each layer 100 events (5GeV mu- gun) | total: ";
   ...
   //gr->Write();
   c1->Print("plots/May_9/mu_5GeV_energy_100_noscint_run1.svg");
