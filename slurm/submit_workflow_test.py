...
#SBATCH --output={out_folder}/%x_NEW.out
#SBATCH --error={error_folder}/%x_NEW.err
...
if [ $(cat {sts_pref}ddsim_status{run_name}.txt) -eq 0 ]; then
    ...
    #########   PROCESS    ##########
    python3 /hpc/group/vossenlab/rck32/eic/work_eic/macros/Timing_estimation/process_root_file.py --filePathName {root_file_dir}/{run_name}_{num_events}.edm4hep.root  --processedDataPath /hpc/group/vossenlab/rck32/eic/work_eic/macros/Timing_estimation/data/processed_data/dev_branch_{num_events}events.csv
    echo $? > {sts_pref}process_status{run_name}.txt
    
    if [ $(cat {sts_pref}process_status{run_name}.txt) -eq 0 ]; then
        echo "Sucessfully processed ROOT file with process_root_file.py"
    else
...
if [ $(cat {sts_pref}process_status{run_name}.txt) -eq 0 ]; then
    echo "Beginning Analysis with analyze_data.py"
...
def submit_simulation_and_processing_jobs_old(num_simulations,simulation_start_num, num_events,particle,hepmc_bool = False):
    particle_name = particle_name_dict[particle]
    current_date = datetime.now().strftime("%B_%d")
    workdir = "/hpc/group/vossenlab/rck32/eic/work_eic"
    slurm_output = f"{workdir}/root_files/Slurm"
    ...
    tensor_path_name = f"{workdir}/macros/Timing_estimation/data/momentum_prediction_pulse/{current_date}_{particle_name}"
    run_name = f"no_inner_old_SIDIS_run_2_{num_events}events"
...
#SBATCH --output={out_folder}/%x_OLD.out
#SBATCH --error={error_folder}/%x_OLD.err
#SBATCH -p vossenlab-gpu
#SBATCH --account=vossenlab
#SBATCH --cpus-per-task=1
#SBATCH --mem=20G
...
#########   DDSIM    ##########
if [ {int(hepmc_bool)} -eq 0 ]; then
    /usr/local/bin/ddsim  --compactFile /hpc/group/vossenlab/rck32/eic/epic_klm/epic_klmws_only.xml --numberOfEvents {num_events} --inputFiles {hepmc_file} --outputFile {root_file_dir}/{run_name}_{num_events}.edm4hep.root  --part.userParticleHandler=""
    echo $? > {sts_pref}ddsim_status{run_name}.txt
else
    /usr/local/bin/ddsim  --compactFile /hpc/group/vossenlab/rck32/eic/epic_klm/epic_klmws_only.xml --numberOfEvents {num_events} --steeringFile {steeringFile} --outputFile {root_file_dir}/{run_name}_{num_events}.edm4hep.root  --part.userParticleHandler=""
    echo $? > {sts_pref}ddsim_status{run_name}.txt
fi
echo ddsim_status: $(cat {sts_pref}ddsim_status{run_name}.txt)
