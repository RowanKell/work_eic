#!/usr/bin/env python3
"""
Memory Stress Test for MOBO Geometries — Preshower Configuration

Tests memory usage across the full MOBO parameter space (preshower_steel_value ×
division_layer_number) to establish safe per-job memory limits and minimize wasted
allocation.

16 configurations = 4 preshower steel values × 4 division layer counts.
3 jobs per (config, particle) combo.

Total: 16 geometries × 3 particles × 3 jobs = 144 jobs

MOBO parameter space (parameters.config):
  preshower_steel_value: 10.0–101.0 mm  (preshower steel layer thickness)
  division_layer_number: 1–7            (integer, number of scintillator divisions)

Fixed geometry constants (not varied):
  HcalScintillatorThickness = 20.0 mm
  HcalScintillatorNbLayers  = 14

Current memory limits in submit_workflow.py (based on fixed scint=20mm, layers=14):
  pi+/kaon0L/proton : 12G  (thin scint ≤30mm, layers>12)
  neutron           :  7G  (thin scint ≤30mm)
  mu-               :  5G  (thin scint ≤30mm)

Worst-case memory hypothesis:
  Min preshower steel (10mm) → more particles reach main layers → more hits → more RAM
  Max division_layer_number (7) → more scintillator segments → more objects → more RAM
...
Usage:
    ...
    python3 /hpc/group/vossenlab/rck32/eic/work_eic/slurm/test_memory_limits_preshower.py --quick
    python3 /hpc/group/vossenlab/rck32/eic/work_eic/slurm/test_memory_limits_preshower.py --analyze-only
"""
...
# 4 preshower steel values × 4 division layer counts = 16 configs
# test_id offset: 9200-series to avoid clashing with existing 9000/9100-series runs
_PRESHOWER_STEEL  = [10.0, 37.0, 68.0, 101.0]   # mm, spans full 10–101 mm MOBO range
_DIVISION_LAYERS  = [1, 3, 5, 7]                  # spans full 1–7 MOBO range

def _build_geometries():
    configs = []
    test_id = 9200
    for div in _DIVISION_LAYERS:
        for ps in _PRESHOWER_STEEL:
            name = f"{div}div_ps{int(ps):03d}"   # e.g. "3div_ps037"
            configs.append((name, test_id, div, ps))
            test_id += 1
    return configs
...
if QUICK_MODE:
    # Worst-case: max division layers + min preshower steel (most hits, most RAM)
    GEOMETRIES = [g for g in ALL_GEOMETRIES if g[2] == 7 and g[3] == 10.0]
else:
...
def create_test_xml(test_id, div_layers, preshower_steel_mm):
    """Create klmws_{test_id}.xml and epic_klmws_only_{test_id}.xml.

    Enforces the total-steel conservation constraint from editxml.py:
      total_steel = HcalSteelThickness * HcalScintillatorNbLayers
      postshower_steel_per_layer = (total_steel - div_layers * preshower_steel_mm)
                                   / (num_layers - div_layers)
    """
    ...
    # Read fixed constants from the template
    steel_per_layer = None
    num_layers = None
    for const in root.iter('constant'):
        if const.get('name') == 'HcalSteelThickness':
            steel_per_layer = float(const.get('value', '').replace('*mm', ''))
        elif const.get('name') == 'HcalScintillatorNbLayers':
            num_layers = int(const.get('value', '0'))

    # Compute postshower_steel_value to keep total steel constant
    total_steel = steel_per_layer * num_layers
    postshower_steel = total_steel - div_layers * preshower_steel_mm
    postshower_steel_per_layer = postshower_steel / (num_layers - div_layers)

    for const in root.iter('constant'):
        name = const.get('name')
        if name == 'division_layer_number':
            const.set('value', str(int(div_layers)))
        elif name == 'preshower_steel_value':
            const.set('value', f'{preshower_steel_mm}*mm')
        elif name == 'postshower_steel_value':
            const.set('value', f'{postshower_steel_per_layer}*mm')
...
def submit_all_tests():
    ...
    log_dir = os.path.join(WORK_EIC, "slurm", "mem_test_logs_preshower")
    os.makedirs(log_dir, exist_ok=True)

    for geo_name, test_id, div_layers, preshower_steel in GEOMETRIES:
        compact_file = create_test_xml(test_id, div_layers, preshower_steel)
        print(f"  Created XML: {geo_name}  div_layers={div_layers}  preshower_steel={preshower_steel}mm")

        for particle in PARTICLES:
            psafe = PARTICLE_SAFE[particle]
            run_name_pref = f"mst_ps_{geo_name}_{psafe}"
...
def get_expected_limit(compact_file, particle):
    """Return the limit that submit_workflow.py would assign for this geometry/particle."""
    return get_mem_limit(compact_file, particle, geo_config="preshower")
...
def analyze_memory(run_names_map):
    print("\n" + "=" * 110)
    print("MEMORY STRESS TEST RESULTS — PRESHOWER CONFIGURATION")
    print(f"  {len(GEOMETRIES)} configs × {len(PARTICLES)} particles × {N_REPS} reps  |  "
          f"MOBO space: preshower_steel 10–101mm, division_layer_number 1–7")
    print("=" * 110)
    ...
    for line in result.stdout.strip().split('\n'):
        ...
        elif '.' not in job_id and 'mst_ps_' in job_name:
    ...
    for job_id, (job_name, state) in parent_info.items():
        m = re.match(r'mst_ps_(.+?)_(pip|mum|neutron)_', job_name)
        if not m:
            continue
        geo_name = m.group(1)
        psafe    = m.group(2)
    ...
    hdr = (f"{'Config':<16} | {'Div':>3} | {'PS Steel':>8} | "
    ...
    for particle in PARTICLES:
        for geo in GEOMETRIES:
            ...
            print(f"{geo_name:<16} | {div_layers:>3} | {preshower_steel:>6.1f}mm | "
    ...
    print()
    # Use worst-case geometry (max div, min preshower steel) as the representative compact file
    worst_geo    = max(GEOMETRIES, key=lambda g: (g[2], -g[3]))  # highest div, lowest steel
    worst_compact = os.path.join(DETECTOR_INSTALL, f"epic_klmws_only_{worst_geo[1]}.xml")
    for particle in PARTICLES:
        ...
        current     = get_expected_limit(worst_compact, particle)
        print(f"  {particle:<8}: max observed = {max_gb:.2f} GB  →  recommended ≥ {recommended}G  (current worst-case limit: {current})")

    print()
    if any_concern:
        print("WARNING: Some configurations have <20% headroom or OOM failures — see (!) flags above.")
        print("         Update get_mem_limit() in submit_workflow.py if limits are insufficient.")
    else:
        print("All configurations have adequate headroom (≥20%). Current limits are safe for preshower config.")
    print("=" * 110)
...
def main():
    print("=" * 110)
    print("MEMORY STRESS TEST FOR MOBO GEOMETRIES — PRESHOWER CONFIGURATION")
    print("=" * 110)
    ...
    if QUICK_MODE:
        print("  (QUICK MODE: worst-case geometry only — div=7, preshower_steel=10mm)")
    if ANALYZE_ONLY:
        print("  (ANALYZE-ONLY: skipping submission, running sacct on today's mst_ps_* jobs)")
    print()
