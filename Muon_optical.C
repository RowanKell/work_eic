#define Muon_optical_cxx
#include "Muon_optical.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void Muon_optical::Loop()

{
   ...
   gr->SetTitle("avg # of hits in each layer 1 event (1GeV mu- gun)");
   ...
   gr->Draw("AP");
   c1->cd(2);
   mc_gr->SetTitle("avg # of -22 PDG in each layer 1 event (1GeV mu- gun)");
   ...
   //Graph stuff
   TFile f("root_files/graphs/May_7/mu_1GeV_energy_1.root","recreate");
   gr->Write();
   c1->Print("plots/May_7/mu_1GeV_energy_1_run1.svg");
   cout << "There were a total of " << count_total << " optical photon hits\n";
   cout << "There were a total of " << mc_count_total << " -22 PDG MCParticles\n";
}
