#define Muon_optical_1GeV_cxx
#include "Muon_optical_1GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void Muon_optical_1GeV::Loop()

{
   ...
   std::string text = "avg # of hits in each layer 100 events (1GeV mu- gun) | total: ";
   text += std::to_string(avg_count_total);
   cout << "text: " << text << "\n";
   
   std::string mc_text = "avg # of -22 PDG in each layer 100 events (1GeV mu- gun) | total: ";
   ...
   c1->Update();
   c1->Draw();
   c1->Print("plots/May_7/mu_1GeV_energy_100_run1.svg");
