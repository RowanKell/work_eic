...
#ifdef PDG_cxx
PDG::PDG(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("root_files/EPIC_KLM_5GeV_mu_1000_no_hadronlist.edm4hep.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("root_files/EPIC_KLM_5GeV_mu_1000_no_hadronlist.edm4hep.root");
      }
...
Int_t PDG::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t PDG::LoadTree(Long64_t entry)
{
