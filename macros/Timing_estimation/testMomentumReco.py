...
def create_output_directories(base_path: str, plotPath: str) -> Dict[str, str]:
...
def main():
    ...
    paths = create_output_directories("/hpc/group/vossenlab/rck32/eic/work_eic/macros/Timing_estimation/",args.plotPath)
    ...
    model_base_path = "/hpc/group/vossenlab/rck32/eic/work_eic/macros/Timing_estimation/models/"
#     model.load(os.path.join(model_base_path, args.inputModelPath))
    try:
        model = Predictor.load(
            os.path.join(model_base_path, args.inputModelPath),
            map_location=device,
            verify=True
        ).to(device)
